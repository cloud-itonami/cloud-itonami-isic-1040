...
(deftest ^{:doc "Soybean oil FFA limits for US jurisdiction."} test-ffa-exceeds-limit-soybean-us
...
(deftest ^{:doc "EU jurisdiction has stricter FFA limits than US."} test-ffa-exceeds-limit-eu-stricter
  (is (registry/ffa-exceeds-limit? 0.4 :soybean-oil "EU")
      "FFA 0.4% should fail EU (limit 0.3%)"))

(deftest ^{:doc "Peroxide value limits per product and jurisdiction."} test-peroxide-value-exceeds-limit
...
(deftest ^{:doc "Temperature range checks."} test-batch-temp-out-of-range
...
(deftest ^{:doc "US holding time limits (90 days = 2160 hours)."} test-holding-time-exceeded-us
...
(deftest ^{:doc "EU has stricter holding time (60 days = 1440 hours)."} test-holding-time-exceeded-eu-stricter
...
(deftest ^{:doc "Sanitation scoring varies by jurisdiction."} test-sanitation-score-insufficient
...
(deftest ^{:doc "Metal detector pass/fail check."} test-metal-detector-pass
...
(deftest ^{:doc "Microbial test pass/fail check."} test-microbial-test-passed
...
(deftest ^{:doc "Holistic batch quality check."} test-batch-quality-acceptable
